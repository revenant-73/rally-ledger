...
describe('RallyLogCard', () => {
  ...
  it('filters match rallies by player involvement, class, serving side, and clears filters', async () => {
    ...
    expect(screen.getByText('Showing 3 of 3 rallies.')).toBeInTheDocument();
    expect(rows()).toHaveLength(3);

    await user.selectOptions(screen.getByLabelText(/Player/i), 'p2');
    expect(screen.getByText('Showing 1 of 3 rallies.')).toBeInTheDocument();
    expect(within(rows()[0]).getByText('#3')).toBeInTheDocument();

    await user.selectOptions(screen.getByLabelText(/Class/i), 'Gifted');
    expect(screen.getByText('Showing 0 of 3 rallies.')).toBeInTheDocument();
    ...
    expect(screen.getByText('Showing 1 of 3 rallies.')).toBeInTheDocument();
    expect(within(rows()[0]).getByText('Kill')).toBeInTheDocument();
  });
});
